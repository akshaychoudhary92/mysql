...
function askForId(rows) {
    inquirer
        .prompt([
            {
                name: 'product_id',
                message: '\n\nType the product ID for the product that you would like to purchase:',
                type: 'input',
...
function askForQuantity(rows, id) {
    ...
    inquirer
        .prompt([
            {
                name: 'quantity',
                message: 'How much "' + product.product_name + '" would you like to purchase?',
                type: 'input',
                validate: validator.isInt
            }
        ])
...
function verifyPurchase(rows, id, quantity) {
    ...
    if (product.stock_quantity < quantity) {
        console.error('\n\nWe don\'t have enough "' + product.product_name + '" for you to buy that many.');
        main();
    } else {
